 ...
 * cycling (e.g. only wilds remain playable) — end the round and score it.
 */
export const STALEMATE_RESHUFFLES = 5;
export const WILD_RANK = CardName.Eight;
export const WILD_VALUE = 50;

/** Penalty value of a card left in hand: wild = 50, T/J/Q/K = 10, A = 1, else pip. */
export function cardValue(
  card: PlayingCard,
  wildRank: CardName = WILD_RANK,
  wildValue: number = WILD_VALUE,
): number {
  if (card.cardName === wildRank) return wildValue;
...
export function handValue(
  ...
  return hand.reduce((sum, c) => sum + cardValue(c, wildRank, wildValue), 0);
}

/**
 * A card is playable if it is wild, matches the active suit, or matches the
 * rank of the top discard.
 */
...
/** Sort a hand by suit then rank, with wilds pulled out to the right. */
export function sortHand(
  hand: PlayingCard[],
  wildRank: CardName = WILD_RANK,
): void {
  hand.sort((a, b) => {
    const aWild = a.cardName === wildRank ? 1 : 0;
    const bWild = b.cardName === wildRank ? 1 : 0;
